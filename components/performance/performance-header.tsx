...
export function PerformanceHeader() {
  ...
  const handleExport = async () => {
    ...
    const data = plans.map((plan: any) => ({
      ...
      Tipo: plan.type,
      Status: plan.status,
      "Usuários Virtuais": plan.virtual_users,
      "Duração (min)": plan.duration_minutes,
      "TPS Esperado": plan.expected_tps,
      "TPS Obtido": plan.actual_tps || "",
      "Tempo Resposta (ms)": plan.response_time_ms || "",
      "Taxa Erro (%)": plan.error_rate || "",
      "Data Execução": formatDateForExcel(plan.execution_date),
      Ferramenta: plan.tool || "",
  ...
  const handleImport = async (data: any[]) => {
    try {
      const plans = data.map((row: any) => ({
        ...
        type: row["Tipo"],
        status: row["Status"],
        virtual_users: row["Usuários Virtuais"],
        duration_minutes: row["Duração (min)"],
        expected_tps: row["TPS Esperado"],
        actual_tps: row["TPS Obtido"],
        response_time_ms: row["Tempo Resposta (ms)"],
        error_rate: row["Taxa Erro (%)"],
